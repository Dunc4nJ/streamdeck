...
```
I need you to SUPER CAREFULLY read absolutely ALL of this and then pick up from where you left off at the end:
```
...
```
Then after extensive exploration, deep analysis, and initial ideas and formulation, please begin an in depth Q and A interview phase where you ask me extensive questions on every aspect of the plan from design specifications to scope and so on. We will follow this loop in the Q & A phase where you will ask questions using the AskUserQuestionTool, review my feedback, deeply analyze, use explore agents where necessary to gather more information based on my feedback, iterate your ideas for the plan, refine, and then ask the next set of questions. Continue this loop until there is zero uncertainty or anything your are unsure about left in your design specifications and scope and I give explicit approval to stop the Q & A phase and begin writing out the full plan.
```
...
```
I want you to scour this entire project high and low looking for any redundant, duplicative code, components, etc.-- especially multiple, conflicting ways of doing the same basic thing. If you uncover anything like that, I want you very very carefully review ALL relevant code files and then find the single best canonical examplar to retain (or combine the best elements you see in terms of code, design, etc) to create a single canonical "best" version of the code file(s) that implement that functioanlity, ensure that it's all properly wired and and connected to the rest of the code and the endpoints/API, and then finally remove completely the now irrelevant "stranded" code to avoid confusion and tech debt in this project. Make sure to carefully read the AGENTS.md file and to follow all the python fastapi best practices shown in the guide referenced in that AGENTS.md file. Make very sure that you aren't, in the course of these changes, breaking any API contracts or losing critical features or functionality.
```
